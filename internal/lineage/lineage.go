// Package lineage provides patrilineal and matrilineal descent tracking.
// It traces the unbroken male-line (Y-chromosome) or female-line
// (mitochondrial) ancestry/descent of any individual in the family.
package lineage
...
// Line represents a unilineal descent path.
type Line struct {
	Type    string   // "patrilineal" or "matrilineal"
	Members []string // ordered from oldest ancestor to person
}

// Patriline traces the unbroken father-line ancestry (Y-chromosome).
// Returns the sequence from the earliest known patrilineal ancestor down to person.
...
// PatrilinealDescendants returns all descendants through unbroken male lines.
func PatrilinealDescendants(f *genealogy.Family, person string) ([]string, error) {
	...
	for len(queue) > 0 {
		...
		for _, child := range children {
			...
			// check if `cur` is a father of child
			...
			if isFather {
				...
				// continue only through male children
				cp, _ := f.Person(child)
				if cp.Sex == "M" {
...
// MostRecentCommonAncestorInLine finds the MRCA through a specific lineage type.
...
// Depth returns the length of the lineage chain (number of generations traced).
func (l *Line) Depth() int {
	return len(l.Members) - 1
}

// Root returns the earliest ancestor in the line.
