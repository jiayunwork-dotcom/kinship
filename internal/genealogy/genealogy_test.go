...
func TestParseFileRejectsBadRecords(t *testing.T) {
	bad := [][]string{
		{"PERSON alice X 1990"},                                            // bad sex
		{"PERSON alice F 19xx"},                                            // bad year
		{"PERSON alice F 1990", "PERSON alice F 1991"},                     // duplicate person
		{"PARENT alice bob"},                                               // unknown parent
		{"PERSON alice F 1990", "PARENT alice bob"},                        // unknown child
		{"PERSON alice F 1990", "PARENT alice alice"},                      // self parent
		{"PERSON a F 1990", "PERSON b M 1990", "PARENT a b", "PARENT a b"}, // dup pair
		{"FAMILY alice"},                                                   // unknown record
		{"# only a comment"},                                               // no data records
	}
...
func TestParseFileForwardReference(t *testing.T) {
	// PARENT appears before the PERSON records it references.
...
func TestLCACases(t *testing.T) {
	...
	cases := []struct {
		...
		{"grace", "george", "george", 3, 0}, // direct line: george is grace's ancestor
		{"emma", "frank", "carl", 1, 1},     // siblings meet at the nearer parent
		{"emma", "ivy", "george", 2, 2},     // first cousins
		{"grace", "ivy", "george", 3, 2},    // first cousins once removed
		{"grace", "henry", "george", 3, 1},  // great-uncle line
	}
	for _, c := range cases {
