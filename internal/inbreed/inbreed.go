// Package inbreed detects and quantifies inbreeding within a pedigree.
// It identifies consanguineous unions, computes pedigree collapse ratios,
// and flags potential genetic risk indicators.
package inbreed
...
// Union represents a parental pair with their kinship coefficient.
type Union struct {
	Parent1    string
	Parent2    string
	Kinship    float64 // kinship coefficient of the parents
	Child      string
	Inbreeding float64 // F of the child = kinship of parents
}

// DetectConsanguineous finds all unions where parents are related.
// Returns unions sorted by inbreeding coefficient (highest first).
...
// ancestors at a given generation depth. A ratio < 1.0 indicates collapse
// (some ancestors appear multiple times).
func PedigreeCollapse(f *genealogy.Family, person string, maxGen int) (float64, error) {
	...
	// count distinct ancestors up to maxGen
	distinct := 0
	for _, d := range anc {
		if d > 0 && d <= maxGen {
			distinct++
	...
	// expected unique ancestors: sum of 2^i for i=1..maxGen
	expected := 0
	for i := 1; i <= maxGen; i++ {
		expected += 1 << i
	}
...
// AverageInbreeding computes the average inbreeding coefficient across all
...
// IsInbred returns true if the person's parents are related.
...
// InbreedingClass classifies the degree of inbreeding.
